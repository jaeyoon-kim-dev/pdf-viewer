...
Highlight fills and temporary text selection use a dedicated SVG blend layer (multiply on light/sepia pages, screen on inverted dark pages). Ink, underlines, selected outlines and sticky memo pins retain a separate normal overlay. Each page isolates blending to its own canvas. Safari pointer capture failures do not abort drag initialization; native canvas dragging is suppressed. Text extraction failures remain visible even when rendering succeeds.
